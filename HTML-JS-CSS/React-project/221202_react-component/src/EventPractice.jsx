...
const EventPractice = () => {
  ...
  */
  ...
  const onChange = (e) => {
    // form을 복사해서 e.target.name에 해당하는 속성만 e.target.value로 수정
    // react에서는 state를 수정할 때 복제해서 수정
    // 하나의 항목으로 만들어진 데이터는 바로 수정하면 되지만
    // 여러 항목으로 구성된 객체나 배열은 복제해서 수정한다.
    // JavaScript에서 객체와 배열을 복제한 후 작업하는 방법에 대해서 알아두어야 한다.
    const nextForm = {
      ...form,
