...
import gmail.sjtxm0320.boardproject.domain.QBoard;
import gmail.sjtxm0320.boardproject.domain.QMember;
import gmail.sjtxm0320.boardproject.domain.QReply;
...
public class SearchBoardRepositoryImpl extends QuerydslRepositorySupport implements SearchBoardRepository {
    ...
    public Board search1() {
        ...
        // member와 join
        // 외래키는 board의 author
        jpqlQuery.leftJoin(member).on(board.author.eq(member));

        // reply와 join
        // 외래키는 reply의 board
        jpqlQuery.leftJoin(reply).on(reply.board.eq(board));

        // 게시글 번호별로 묶어서 board와 memeber의 email, reply의 개수 가져오기
        /*
